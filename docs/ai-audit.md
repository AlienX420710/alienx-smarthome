...
5. **Streamed body-size bounding** — the body is read via
   ...
   the moment cumulative bytes exceed 16 KB, _before_ `JSON.parse` ever
...
7. **In-memory rate limiting** — a `Map<ip, timestamp[]>` keyed by
   ...
   map is at capacity, a _new_ IP is rejected rather than allowed to grow
...
- Derives the Resend `Idempotency-Key` from a SHA-256 digest of
  `[requestId, details, message]`, so retries of the _same_ logical
...
says _"Configuration check only; no inquiry or provider delivery test is
performed"_. It returns HTTP 503 when any check is degraded (so uptime
