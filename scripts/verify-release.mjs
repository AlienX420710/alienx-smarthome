...
if (!/^[a-f0-9]{40}$/.test(expected ?? '')) throw new Error('EXPECTED_REVISION must be a full Git SHA');
...
while (Date.now() < deadline) {
  try {
    ...
    if (response.ok && data.ok === true && data.status === 'operational' && data.buildRevision === expected) {
      console.log(`Verified healthy production revision ${expected}; rate limiting: ${data.rateLimiting}`);
      process.exit(0);
    }
