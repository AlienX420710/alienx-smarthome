...
function boot(saved = {}, blocked = false) {
  const handlers = {};
  const document = { documentElement: { dataset: {} }, addEventListener: (event, handler) => { handlers[event] = handler; }, dispatchEvent() {} };
  const window = {};
  vm.runInNewContext(fs.readFileSync('public/site-preferences.js', 'utf8'), { document, window, URL, Event, location: { href: 'https://example.test/contact/' }, localStorage: { getItem: key => { if (blocked) throw Error(); return saved[key]; }, setItem: (key, value) => { if (blocked) throw Error(); saved[key] = value; } } });
...
test('system choice clears explicit theme before a navigation swap', () => {
  ...
  handlers['astro:before-swap']({ newDocument: next, to: new URL('https://example.test/experience/') });
...
test('explicit theme survives navigation even when preference storage is blocked', () => {
  ...
  handlers['astro:before-swap']({ newDocument: next, to: new URL('https://example.test/technology') });
