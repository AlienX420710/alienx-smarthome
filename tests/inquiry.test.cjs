...
function load(file, extra = {}) {
  ...
  const context = { exports: {}, Response, Request, Headers, URL, URLSearchParams, TextDecoder, TextEncoder, Uint8Array, AbortSignal, crypto, console, defineMiddleware: f => f, ...extra };
...
function harness(provider = { id: 'mock-id' }, bindings = {}) {
  ...
  const sent = [];
  const middleware = load('src/middleware.ts', { mockBindings: { TURNSTILE_SECRET: 'test', TURNSTILE_HOSTNAMES: 'example.test', ...bindings }, fetch: async () => { verifies++; return Response.json({ success: true, action: 'contact', hostname: 'example.test' }); } }).onRequest;
  const endpoint = load('src/pages/api/inquiry.ts', { env: { RESEND_API_KEY: 'test' }, fetch: async (_url, options) => { emails++; sent.push(options); return Response.json(provider); } }).POST;
  return { counts: () => ({ verifies, emails }), sent, endpoint, async send(body, path = '/api/inquiry', headers = {}) {
...
test('provider retry key and email body stay stable across retries', async () => {
  ...
  await h.send({ ...valid, message: 'A different valid inquiry.' }, undefined, headers);
  assert.notEqual(h.sent[0].headers['Idempotency-Key'], h.sent[2].headers['Idempotency-Key']);
});
test('edge limiter rejection and outage fail closed before providers', async () => {
  for (const [limit, expected] of [[async () => ({ success: false }), 429], [async () => { throw Error('Unavailable'); }, 503]]) {
...
test('retry key shape is bounded and validated', async () => {
  const h = harness();
  assert.equal((await h.send(valid, undefined, { 'Idempotency-Key': 'bad' })).status, 400);
  assert.deepEqual(h.counts(), { verifies: 0, emails: 0 });
});
